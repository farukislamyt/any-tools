...
export default function DocumentEditorPage() {
  ...
  const deleteDocument = () => {
    if (documents.length <= 1) {
      window.alert("Keep at least one document in the editor.");
      return;
    }
  ...
  const replaceAll = () => {
    ...
    setMatchCount(replaced);
    updateStats();
    if (replaced) scheduleSave();
  };

  useEffect(() => {
    setMatchCount(findMatches(findText));
  }, [findText, matchCase, documents, activeId]);
