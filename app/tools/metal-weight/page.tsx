...
export default function MetalWeightPage() {
  ...
  const weight = useMemo(() => {
    const d = Number(dimension), l = Number(length);
    if (!d || !l || d < 0 || l < 0) return 0;
    const area = shape === "round" ? Math.PI * (d / 2) ** 2 : shape === "square" ? d * d : d * 10;
    return (area * l * materials[material]) / 1e9;
  }, [dimension, length, material, shape]);
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <label className="text-sm font-medium text-slate-700">Size (mm)<input inputMode="decimal" value={dimension} onChange={e => setDimension(e.target.value)} className="mt-2 h-12 w-full rounded-xl border border-slate-200 px-3 outline-none focus:border-blue-500" /></label>
        ...
        <p className="mt-6 text-xs leading-5 text-slate-400">Approximate result using standard material density. For flat bars, size is interpreted as width × 10 mm thickness in this first version.</p>
      </section>
